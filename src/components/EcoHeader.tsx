...
import { localGetCurrentUser, localSignOut } from "@/services/localDb";
...
export default function EcoHeader({ schoolName, schoolLogo }: EcoHeaderProps = {}) {
  ...
  const handleSignOut = async () => {
    if (isLocalMode) {
      localSignOut();
      toast({ title: "Ate logo", description: "Logout local realizado." });
      navigate("/auth");
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: "Ate logo", description: "Logout realizado com sucesso." });
      navigate("/auth");
    }
  };
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          <div className="w-9 h-9 md:w-11 md:h-11 rounded-xl bg-white/90 flex items-center justify-center flex-shrink-0 p-1">
            <img
              ...
              alt={schoolName ? `Logo ${schoolName}` : "OEP Sustentavel"}
